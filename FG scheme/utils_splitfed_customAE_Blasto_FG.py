...
def inf(loader, modelclientFE,modelserver,modelclientBE,comp_model1,comp_model2,loss_fn,folder):
    inf_running_loss = 0.0
    inf_iou_score_class0 = 0.0
    inf_iou_score_class1 = 0.0
    inf_iou_score_class2 = 0.0
    inf_iou_score_class3 = 0.0
    inf_iou_score_class4 = 0.0
    inf_running_totrate = 0.0
    inf_running_rate1 = 0.0
    inf_running_rate2 = 0.0
    thinf_running_rate1 = 0.0
    thinf_running_rate2 = 0.0
    inf_running_mseloss1 = 0.0
    inf_running_mseloss2 = 0.0
    inf_running_correct = 0.0
    inf_running_totdiceloss = 0.0
    inf_accuracy = 0.0
    inf_f1_score = 0.0
    lambda1=100
    with torch.no_grad():
        for idx,(x, y) in enumerate(loader):
            ...
            #actual compression
            enc_pred1 = comp_model1.encode(predictions1)
            string1 = comp_model1.entropy_bottleneck.compress(enc_pred1)
            bpp1 = sum(len(s) for s in string1) * 8.0 / x.numel()
            
            #actual decompression
            y_hat1 = comp_model1.entropy_bottleneck.decompress(string1, enc_pred1.size()[-2:])
            x_hat1 = comp_model1.decode(y_hat1)
            mse_loss1 = F.mse_loss(predictions1, x_hat1)
            S1_totloss = bpp1  + lambda1*(mse_loss1)
            
            #------------Theoretical bitrate----------------------
            _, y_likelihoods1 = comp_model1.forward(predictions1)
            N, C, H, W = x.size()
            th_bpp1 = torch.log(y_likelihoods1).sum() / (-math.log(2) * N*H*W)
            #----------------------------------------------------
            
            predictions2 = modelserver(x_hat1)
            comp_model2.entropy_bottleneck.update(force=True)
            
            #actual compression
            enc_pred2 = comp_model2.encode(predictions2)
            string2 = comp_model2.entropy_bottleneck.compress(enc_pred2)
            bpp2 = sum(len(s) for s in string2) * 8.0 / x.numel()
            
            #actual decompression
            y_hat2 = comp_model2.entropy_bottleneck.decompress(string2, enc_pred2.size()[-2:])
            x_hat2 = comp_model1.decode(y_hat2)
            mse_loss2 = F.mse_loss(predictions2, x_hat2)
            S2_totloss = bpp2  + lambda1*(mse_loss2)
            
            #------------Theoretical bitrate----------------------
            _, y_likelihoods2 = comp_model2.forward(predictions2)
            N, C, H, W = x.size()
            th_bpp2 = torch.log(y_likelihoods2).sum() / (-math.log(2) * N*H*W)
            #----------------------------------------------------
            ...
            preds = torch.argmax(predictions3, dim=1)
            equals = preds == y
            inf_running_correct += torch.mean(equals.type(torch.FloatTensor)).item()
            inf_running_loss += loss.item()
            inf_running_mseloss1 += mse_loss1.item()
            inf_running_mseloss2 += mse_loss2.item()
            inf_running_rate1 += bpp1
            inf_running_rate2 += bpp2
            thinf_running_rate1 += th_bpp1
            thinf_running_rate2 += th_bpp2
            inf_running_totrate += bpp1 + bpp2
            inf_running_totdiceloss +=    dice_loss3.item()
            
            inf_f1_score += f1_score(y.cpu().flatten(), preds.cpu().flatten(), average='micro')
            inf_accuracy += accuracy_score(y.cpu().flatten(), preds.cpu().flatten())
            iou_sklearn = jaccard_score(y.cpu().flatten(), preds.cpu().flatten(), average=None)
            inf_iou_score_class0 += iou_sklearn[0]
            inf_iou_score_class1 += iou_sklearn[1]
            inf_iou_score_class2 += iou_sklearn[2]
            inf_iou_score_class3 += iou_sklearn[3]
            inf_iou_score_class4 += iou_sklearn[4]
            torchvision.utils.save_image(preds.float(), f"{folder}/pred_{idx}.BMP", padding=0, scale_each=True,normalize=True)

    epoch_loss = inf_running_loss / len(loader.dataset)
    epoch_totrate = inf_running_totrate / len(loader.dataset)
    epoch_rate1 = inf_running_rate1 / len(loader.dataset)
    epoch_rate2 = inf_running_rate2 / len(loader.dataset)
    epoch_th_rate1 = thinf_running_rate1 / len(loader.dataset)
    epoch_th_rate2 = thinf_running_rate2 / len(loader.dataset)
    epoch_mseloss1 = inf_running_mseloss1 / len(loader.dataset)
    epoch_mseloss2 = inf_running_mseloss2 / len(loader.dataset)
    epoch_totdiceloss = inf_running_totdiceloss / len(loader.dataset)
    epoch_acc = 100. * (inf_running_correct / len(loader.dataset))
    epoch_iou_class0 = (inf_iou_score_class0 / len(loader.dataset))
    epoch_iou_class1 = (inf_iou_score_class1 / len(loader.dataset))
    epoch_iou_class2 = (inf_iou_score_class2 / len(loader.dataset))
    epoch_iou_class3 = (inf_iou_score_class3 / len(loader.dataset))
    epoch_iou_class4 = (inf_iou_score_class4 / len(loader.dataset))
    epoch_iou_withbackground = (epoch_iou_class0 + epoch_iou_class1 + epoch_iou_class2 + epoch_iou_class3 + epoch_iou_class4) / 5
    epoch_iou_nobackground = (epoch_iou_class1 + epoch_iou_class2 + epoch_iou_class3 + epoch_iou_class4) / 4
    print("inf accuracy score:",epoch_acc)
    print("inf mean IoU withbackground:",epoch_iou_withbackground)
    print("inf mean IoU withoutbackground:",epoch_iou_nobackground) #needed_prediction
    print ("IoU of Background:", epoch_iou_class0)
    print ("IoU of ZP:", epoch_iou_class1)
    print ("IoU of TE:", epoch_iou_class2)
    print ("IoU of ICM:", epoch_iou_class3)
    print ("IoU of Blastocoel:", epoch_iou_class4)
    print("inf epoch mse_loss1:", epoch_mseloss1)
    print("inf epoch mse_loss2:", epoch_mseloss2)
    print("inf epoch bpp1:", epoch_rate1)
    print("infepoch bpp2:", epoch_rate2)
    print("infepoch th_bpp1:", epoch_th_rate1)
    print("infepoch th_bpp2:", epoch_th_rate2)
    print("infepoch total bpp:", epoch_totrate)
    print("inf epoch total dice_loss:", epoch_totdiceloss)
    return epoch_loss, epoch_acc, epoch_iou_withbackground,epoch_iou_nobackground