...
def test(loader, modelclientFE,modelserver,modelclientBE,comp_model1,comp_model2,loss_fn,folder):
    ...
    test_running_totrate = 0.0
    test_running_rate1 = 0.0
    test_running_rate2 = 0.0
    ...
    with torch.no_grad():
        for idx,(x, y) in enumerate(loader):
            ...
            #------------Theoretical bitrate----------------------
            x_hat1, y_likelihoods1 = comp_model1.forward(predictions1)
            N, C, H, W = x.size()
            num_pixels = N * H * W 
            bpp1 = torch.log(y_likelihoods1).sum() / (-math.log(2) * num_pixels)
            mse_loss1 = F.mse_loss(predictions1, x_hat1)
            S1_totloss = bpp1  + lambda1*(mse_loss1)
            #----------------------------------------------------
            predictions2 = modelserver(x_hat1)
            #------------Theoretical bitrate----------------------
            x_hat2, y_likelihoods2 = comp_model2.forward(predictions2)
            bpp2 = torch.log(y_likelihoods2).sum() / (-math.log(2) * num_pixels)
            mse_loss2 = F.mse_loss(predictions2, x_hat2)
            ...
            test_running_rate1 += bpp1
            test_running_rate2 += bpp2
            test_running_totrate += bpp1 + bpp2
    ...
    epoch_totrate = test_running_totrate / len(loader.dataset)
    epoch_rate1 = test_running_rate1 / len(loader.dataset)
    epoch_rate2 = test_running_rate2 / len(loader.dataset)
    ...
    print("TESTING epoch bpp1:", epoch_rate1)
    print("TESTINGepoch bpp2:", epoch_rate2)
    print("TESTINGepoch total bpp:", epoch_totrate)
